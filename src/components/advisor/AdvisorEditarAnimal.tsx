import { ArrowLeft, Upload, X, Camera, FileText, Plus } from 'lucide-react';
...
interface AdvisorEditarAnimalProps {
  animal: Animal;
  onBack: () => void;
  onSave: () => void;
}
...
export function AdvisorEditarAnimal({ animal, onBack, onSave }: AdvisorEditarAnimalProps) {
  const [formData, setFormData] = useState({
    ...
    sexo: animal.sex || 'Macho',
    dataNascimento: animal.birthDate || '2020-01-15',
    pelagem: 'Alazã',
    andamento: 'Marchado',
    pai: '',
    mae: '',
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  className="w-full h-10 px-4 bg-muted dark:bg-[#0d0d0d] border border-border dark:border-[rgba(255,255,255,0.1)] rounded-lg text-foreground dark:text-white focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-white/20"
                  placeholder="Ex: Relâmpago"
                />
              </div>

              <div>
                <label className="block text-sm text-foreground dark:text-white mb-2">Registro *</label>
                <input
                  type="text"
                  ...
                  {existingMedia.map((item) => (
                    <div key={item.id} className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full h-10 px-3 bg-muted dark:bg-[#0d0d0d] border border-border dark:border-[rgba(255,255,255,0.1)] rounded-xl text-sm text-foreground dark:text-white focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-white/20"
                      />
                    </div>
                  ))}
                </div>
